...
class AcademicSystemApp(tk.Tk):
    ...
    def open_scorecard_window(self):
        ...
        export_button = tk.Button(win, text="Export to PDF", command=lambda: self.export_to_pdf(self.current_scorecard_fig, "scorecard"))
        ...
        def delayed_draw():
            try:
                ...
                self.current_scorecard_fig = fig
    ...
    def save_schedule(self, popup, teacher_id_entry, teacher_name_entry, class_id_entry, class_name_entry, day_vars, schedule_tree):
        try:    
            ...
            for day in selected_days:
                cursor.execute("SELECT COUNT(*) FROM Schedule WHERE TeacherID=%s AND ClassID=%s AND DaysTeach=%s", (teacher_id, class_id, day))
                result = cursor.fetchone()
                if result and result[0] > 0:
                    messagebox.showerror("Duplicate Entry", f"Schedule for {teacher_id} - {class_id} on {day} already exists.")#"This teacher-class pair already has a schedule.")
                    return

            # Insert one row per selected day
            for day in selected_days:
                cursor.execute(
                    "INSERT INTO Schedule (TeacherID, ClassID, DaysTeach) VALUES (%s, %s, %s)",
                    (teacher_id, class_id, day)
                )
